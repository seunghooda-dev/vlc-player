...
from file_list_view import (
    FILE_ITEM_HTML_ROLE, FILE_ITEM_PLAIN_ROLE,
    FILE_FILTER_KEYS, FILE_FILTER_VISIBLE_KEYS, FILE_FILTER_LABELS, FILE_FILTER_TIPS,
...
class RightPanel(QWidget):
    ...
    def _qc_report_rows(self, files=None):
        ...
        for f in report_files:
            ...
            row = {
                '앱버전': f'MasterQC Player v{APP_VERSION}',
    ...
    def _write_qc_report_txt(self, path, rows):
        ...
        lines = []
        lines.append(f'MasterQC Player v{APP_VERSION} - QC 결과 리포트')
        lines.append('=' * 64)
    ...
    def _issue_summary_clipboard_text(self, files=None):
        ...
        lines = ["MasterQC Player 확인 필요 파일 요약"]
        try:
    ...
    def _next_qc_issue_seek_time(self, record, current_sec=0.0):
        starts = self._qc_issue_seek_times(record)
        if not starts:
            return None
    ...
    def _seek_next_qc_issue(self, record, filepath=None):
        ...
        try:
            ...
            current_sec = 0.0
        target = self._next_qc_issue_seek_time(record, current_sec)
        if target is None:
            self.vp.status_changed.emit(f"  확인할 문제 구간이 없습니다 — {self._path_name(fp)}")
            return False
        self.seek_requested.emit(float(target))
